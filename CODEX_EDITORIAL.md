...
8. Wait for the Pages deployment and the weekly WeCom workflow. A green workflow is not proof of delivery because `--allow-not-ready` can turn a readiness skip into a successful run. Fetch `origin/main` and require `data/state/deliveries.json` to contain the current week's exact digest before reporting delivery. Diagnose and repair missing Chinese fields or a stale deployed digest, but never use `--force` to bypass duplicate protection.

The push and the matching WeCom delivery record are required acceptance checks, independent of whether the individual workflows appear green. Authentication failures, network failures, dirty worktrees, rebase conflicts, missing webhook configuration, and deployment failures must be reported as failed runs rather than hidden; external infrastructure can still prevent guaranteed delivery. Friday 19:00 remains a fallback check.
