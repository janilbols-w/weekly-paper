...
推送工作流由 Pages 部署成功事件触发。会议数据或会议通知工作流提交时也会先检查一次；如果页面尚未更新便安全跳过，Pages 部署完成后自动重试。周报在周五 19:00、会议专题在周五 22:00 另有兜底检查。Webhook 配置或真实发送错误仍会使工作流失败。发送前会检查对应页面的线上 digest，成功后分别在 `data/state/deliveries.json` 和 `data/state/event-deliveries.json` 记录摘要哈希以防重复投递。会议通知默认只检查最近 7 天内最后生成的专题，避免首次启用时补发历史会议。
